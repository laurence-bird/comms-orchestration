...
class ChannelSelectorSpec extends FlatSpec with Matchers with ArbGenerator {
  ...
  it should "Return email if there are no trigger channel preferences" in {
    val triggered = triggeredBase.copy(preferredChannels = None)
    val channelResult =
      ChannelSelector.determineChannel(retrieveTemplate(smsAndEmailTemplate))(customerProfile, triggered)

    channelResult shouldBe Right(Email)
  }

  it should "Return the highest priority trigger channel preference if it is available" in {
  ...
  it should "Return the second trigger channel preference if the first is not available" in {
    val triggered     = triggeredBase.copy(preferredChannels = Some(List(Email, SMS)))
    val channelResult = ChannelSelector.determineChannel(retrieveTemplate(smsOnlyTemplate))(customerProfile, triggered)
