...
import com.ovoenergy.orchestration.scheduling.Persistence.Successful
...
class DynamoPersistenceSpec extends FlatSpec with Matchers with ArbGenerator {
  ...
  it should "cancel pending and expired orchestrating schedules" in {
    LocalDynamoDB.withTableWithSecondaryIndex(client, tableName)(Seq('scheduleId -> S))(secondaryIndices) {
      ...
      val pendingSchedule = generate[Schedule].copy(history = List(), customerId = customerId, commName = commName, status = ScheduleStatus.Pending)
      val expiredOrchestratingSchedule = generate[Schedule].copy(history = List(), customerId = customerId, commName = commName, status = ScheduleStatus.Orchestrating, orchestrationExpiry = now.minusSeconds(60 * 30))
      val inProgressOrchestratingSchedule = generate[Schedule].copy(customerId = customerId, commName = commName, status = ScheduleStatus.Orchestrating, orchestrationExpiry = now.plusSeconds(60 * 10))
      persistence.storeSchedule(completedSchedule)
      persistence.storeSchedule(pendingSchedule)
      persistence.storeSchedule(expiredOrchestratingSchedule)
      persistence.storeSchedule(inProgressOrchestratingSchedule)
      ...
      persistence.retrieveSchedule(inProgressOrchestratingSchedule.scheduleId.toString).get shouldBe inProgressOrchestratingSchedule

      val cancelledPendingSchedule = pendingSchedule.copy(status = ScheduleStatus.Cancelled, history = List(Change(now, "Cancelled")))
      persistence.retrieveSchedule(pendingSchedule.scheduleId.toString).get shouldBe cancelledPendingSchedule

      val cancelledExpiredOrchestratingSchedule = expiredOrchestratingSchedule.copy(status = ScheduleStatus.Cancelled, history = List(Change(now, "Cancelled")))
