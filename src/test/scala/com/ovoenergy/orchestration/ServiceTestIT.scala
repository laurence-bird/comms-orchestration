...
class ServiceTestIT extends FlatSpec with Matchers with ScalaFutures with BeforeAndAfterAll with IntegrationPatience {
  ...
  def setupTopics() {
    ...
    if (!AdminUtils.topicExists(zkUtils, emailOrchestratedTopic))
      AdminUtils.createTopic(zkUtils, emailOrchestratedTopic, 1, 1)
    if (!AdminUtils.topicExists(zkUtils, cancellationRequestTopic))
      AdminUtils.createTopic(zkUtils, cancellationRequestTopic, 1, 1)
    if (!AdminUtils.topicExists(zkUtils, failedCancellationTopic))
      AdminUtils.createTopic(zkUtils, failedCancellationTopic, 1, 1)
  ...
  def createInvalidCustomerProfileResponse() {
    val validResponseJson =
      new String(Files.readAllBytes(Paths.get("src/test/resources/profile_missing_required_fields_response.json")),
                 StandardCharsets.UTF_8)
