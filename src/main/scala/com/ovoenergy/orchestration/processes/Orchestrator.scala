...
trait Orchestrator[F[_]] {
  def apply(triggeredV4: TriggeredV4, internalMetadata: InternalMetadata): F[Either[ErrorDetails, RecordMetadata]]
}
...
object Orchestrator extends LoggingWithMDC {
  ...
  def apply[F[_]: Async](channelSelector: ChannelSelector[F],
    ...
    new Orchestrator[F] {
      override def apply(triggered: TriggeredV4,
                         internalMetadata: InternalMetadata): F[Either[ErrorDetails, RecordMetadata]] = {
  ...
  // TODO: Clean up main by creating a resource method similar to CustomerProfiler
}
